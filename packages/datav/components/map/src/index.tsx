  ...
  setup() {
    ...
    async function handleStationDistribution(area_code: string) {
      ...
      if (typeof area_code !== 'string') {
        throw new TypeError('area_code must be a string')
      }
    ...
    return () => (
      <VChart ref={chartRef.value} class="chart" option={option.value} autoresize onClick={handleClick} onDblclick={handleDbClick} />
    )
  },
})
